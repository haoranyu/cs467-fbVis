...
<div id="left">
		...
		<div id="rel">
			<table style="font-size:11px;width:60px;border-right:1px solid #ddd;text-align:right;color:#999">
				...
				<tr>
					<td>
					<?php if($i%3==0){?>
					<?php echo	intval($i/6)==0?'12':intval($i/6)?>:
					<?php }?>
					<?php if($i%6==0){?>
					<?php echo	'00';?>
					<?php }elseif($i%3==0){?>
	...
	<div class="cate">
		 ...
		 <span style="background:yellowgreen">Photo/Video</span>
...
$(".list li").click(function(){
	...
	//	$("[uid='"+$(this).attr('id')+"']").css('background','');
		$("[uid='"+$(this).attr('id')+"']").show();
		$("[uid='"+$(this).attr('id')+"']").parent().parent().css('background','');
	}
});
$("#sel").click(function(){
	$(".list li.check").click();
